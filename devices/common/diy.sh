...
sed -i "s/192.168/10.10/" package/base-files/files/bin/config_generate
...
coremark wget-ssl curl autocore htop nano zram-swap kmod-lib-zstd kmod-tcp-bbr bash openssh-sftp-server block-mount resolveip ds-lite swconfig luci-app-fan luci-nginx /" include/target.mk
